...
export async function importAppfolioCoa(db, organizationId, options = {}) {
  ...
  await db.transaction(async (tx) => {
    ...
    for (const item of [...roots, ...children]) {
      ...
      // functional, etc.) from lib/accounting/defaultGlAccountTags.js
      // rules. Idempotent — safe inside re-runs / partial imports.
      await applyDefaultTagsForAccount(tx, row.id, {
        code: item.code,
        name: item.name,
        accountType: item.accountType,
