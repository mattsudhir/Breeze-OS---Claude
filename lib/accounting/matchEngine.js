...
export async function confirmMatchCandidate(
  ...
  // cash side of the posting). Also pull bill_com_transaction_id so
  // we can fire a write-back after posting.
  const [txn] = await tx
    .select({
      ...
      billComTransactionId: bankTransactions.billComTransactionId,
    })
    .from(bankTransactions)
    .leftJoin(
      bankAccounts,
  ...
  // Bill.com write-back. When the transaction came from a Bill.com
  // card sync AND the target GL is mapped to a Bill.com COA id, push
  // the categorization back so their bookkeeper-facing surface shows
  // the same coding we just posted. Best-effort: a failure does NOT
  // roll back our JE — that work is already correct in Breeze.
  await maybeWriteBackToBillCom(tx, organizationId, {
    txn, targetGlAccountId, memo,
  });
...
// ── Bill.com write-back helper ─────────────────────────────────

/**
 * Push a categorization back to Bill.com when:
 *   1. The bank_transaction has a bill_com_transaction_id (came from
 *      Bill.com's card sync), AND
 *   2. The org's bookkeeper_review_location is 'bill_com' or 'both', AND
 *   3. The target GL has a bill_com_chart_of_accounts_id mapping.
 *
 * Best-effort. If anything fails, log and continue — the JE in our
 * books is correct and that's what matters.
 */
async function maybeWriteBackToBillCom(tx, organizationId, { txn, targetGlAccountId, memo }) {
  try {
    if (!txn?.billComTransactionId) return;

    const [org] = await tx
      .select({ location: organizations.bookkeeperReviewLocation })
      .from(organizations)
      .where(eq(organizations.id, organizationId))
      .limit(1);
    const location = org?.location || 'breeze';
    if (location !== 'bill_com' && location !== 'both') return;

    const [gl] = await tx
      .select({ billComCoaId: glAccounts.billComChartOfAccountsId })
      .from(glAccounts)
      .where(eq(glAccounts.id, targetGlAccountId))
      .limit(1);
    if (!gl?.billComCoaId) return;

    const { markCardTransactionReviewed, isBillComConfigured } = await import(
      '../backends/billcom.js'
    );
    if (!isBillComConfigured()) return;

    await markCardTransactionReviewed({
      billComTransactionId: txn.billComTransactionId,
      chartOfAccountsId: gl.billComCoaId,
      memo,
      reviewStatus: 'reviewed',
    });
  } catch (err) {
    console.warn(
      `[match-engine] Bill.com write-back failed for txn ${txn?.id}: ${err.message}`,
    );
  }
}
