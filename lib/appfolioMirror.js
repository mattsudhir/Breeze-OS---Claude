...
export async function readListFromMirror(
  ...
  // PropertyName, and Rent blank — those fields live on Unit /
  // Occupancy. We already mirror Units, so patch tenant rows with
  // unit_name + property_name + market_rent from the Units mirror
  // before returning. (Rent here is the unit's market rent — close
  // enough for a list card; actual lease rent needs occupancies,
  // which we don't mirror yet.)
    ...
    if (unitIds.length > 0) {
      const unitRows = await db
        ...
        .where(and(
          ...
          inArray(schema.appfolioCache.resourceId, unitIds.map(String)),
        ));
      ...
      records = records.map((t) => {
        if (!t.unit_id) return t;
        const u = unitById.get(String(t.unit_id));
        if (!u) return t;
        return {
          ...t,
          unit_name: t.unit_name || u.name || '',
          property_name: t.property_name || u.property_name || '',
          rent: t.rent || u.market_rent || null,
        };
      });
    }
  }
