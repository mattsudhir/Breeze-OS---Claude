...
function mapAppfolioOccupancyRow(o) {
  if (!o) return null;
  return {
    id: o.Id || o.id,
...
const TYPE_CONFIG = {
  ...
  occupancy: {
    appfolioEndpoint: '/occupancies',
    listTool: 'list_occupancies',
    responseField: 'occupancies',
    listInput: { active_only: false, limit: 10000 },
    mapAppfolioToCanonical: mapAppfolioOccupancyRow,
    extractIndex: (row) => ({
      propertyId: row.property_id || null,
      unitId: row.unit_id || null,
      occupancyId: row.id || null,
      status: row.status || null,
      hiddenAt: row.hidden ? new Date() : null,
    }),
  },
...
export async function readListFromMirror(
  ...
  let records = rows.slice(0, cap).map((r) => r.data);
  const hasMore = rows.length > cap;

  // Enrichment: AppFolio's /tenants list response leaves UnitName,
  // PropertyName, Rent, MoveInDate, LeaseStart/End all blank —
  // those fields live on Unit + Occupancy. We mirror both, so
  // patch tenant rows from those tables before returning. Two
  // joins:
  //   - Units mirror keyed on tenant.unit_id  → unit_name +
  //     property_name (plus market_rent as a fallback for tenants
  //     not yet linked to an occupancy).
  //   - Occupancies mirror keyed on tenant.occupancy_id → actual
  //     lease rent + move_in_date + lease_start/end. Occupancy
  //     wins over the unit's market_rent when both exist.
  if (resourceType === 'tenant' && records.length > 0) {
    const unitIds = [...new Set(
      records.map((t) => t.unit_id).filter(Boolean),
    )].map(String);
    const occupancyIds = [...new Set(
      records.map((t) => t.occupancy_id).filter(Boolean),
    )].map(String);

    const [unitRows, occupancyRows] = await Promise.all([
      unitIds.length > 0
        ? db
            .select({ data: schema.appfolioCache.data })
            .from(schema.appfolioCache)
            .where(and(
              ...
              inArray(schema.appfolioCache.resourceId, unitIds),
            ))
        : Promise.resolve([]),
      occupancyIds.length > 0
        ? db
            ...
            .where(and(
              eq(schema.appfolioCache.organizationId, organizationId),
              eq(schema.appfolioCache.resourceType, 'occupancy'),
              inArray(schema.appfolioCache.resourceId, occupancyIds),
            ))
        : Promise.resolve([]),
    ]);
    ...
    for (const r of unitRows) {
      if (r.data?.id) unitById.set(String(r.data.id), r.data);
    }
    const occupancyById = new Map();
    for (const r of occupancyRows) {
      if (r.data?.id) occupancyById.set(String(r.data.id), r.data);
    }

    records = records.map((t) => {
      const u = t.unit_id ? unitById.get(String(t.unit_id)) : null;
      const o = t.occupancy_id ? occupancyById.get(String(t.occupancy_id)) : null;
      if (!u && !o) return t;
      return {
        ...t,
        unit_name: t.unit_name || u?.name || '',
        property_name: t.property_name || u?.property_name || '',
        rent: t.rent || o?.rent || u?.market_rent || null,
        move_in_date: t.move_in_date || o?.move_in_date || null,
        move_out_date: t.move_out_date || o?.move_out_date || null,
        lease_start: t.lease_start || o?.lease_start || null,
        lease_end: t.lease_end || o?.lease_end || null,
      };
    });
  }
