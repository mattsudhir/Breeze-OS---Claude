...
//   ZOHO_CLIQ_WEBHOOK_URL – incoming webhook URL for the target Cliq bot
//     or channel. Can be set either with the zapikey already appended
//     (e.g. ".../incoming?zapikey=xxx") or as the bare base URL. If the
//     URL has no zapikey query param and ZOHO_CLIQ_BOT_TOKEN is set, we
//     append it automatically so callers don't have to duplicate the
//     token across two env vars.
//   ZOHO_CLIQ_BOT_TOKEN – optional; the shared token used both as the
//     inbound /api/cliq verification secret and, if needed, as the
//     outbound webhook's zapikey.
//
...
// Build the final outbound URL by merging ZOHO_CLIQ_WEBHOOK_URL with
// ZOHO_CLIQ_BOT_TOKEN as the zapikey if the URL doesn't already carry
// one. Exported for tests; regular callers use postToCliq.
export function resolveWebhookUrl() {
  const base = process.env.ZOHO_CLIQ_WEBHOOK_URL;
  if (!base) return null;

  // Fast path: caller already baked in ?zapikey=... (or any auth param).
  if (/[?&]zapikey=/.test(base)) return base;

  const token = process.env.ZOHO_CLIQ_BOT_TOKEN;
  if (!token) return base; // let Cliq reject it so the error is visible

  const sep = base.includes('?') ? '&' : '?';
  return `${base}${sep}zapikey=${encodeURIComponent(token)}`;
}
