...
// ── Charge card (Spend & Expense) transactions ─────────────────
//
// Bill.com's Spend & Expense product (formerly Divvy) exposes
// card transactions as "ChargeAccountTransaction" entities. Each
// transaction has:
//   id, vendorName, amount, transactionDate,
//   reviewStatus ('needsReview' | 'reviewed' | 'rejected'),
//   chartOfAccountsId (their internal GL id), department, location,
//   memo
//
// Endpoint surface used here is approximate — Bill.com's exact API
// shape for cards has shifted across product versions. The contract
// below treats their endpoint as "GET list of transactions" and
// "POST status update with categorization." Adjust field names when
// we wire against a real account.

/**
 * List charge-card transactions, optionally filtered by review status.
 *
 * @param {object} opts
 * @param {string} [opts.reviewStatus]  'needsReview' | 'reviewed' | etc.
 * @param {string} [opts.sinceDate]     ISO timestamp; transactions after this
 * @param {number} [opts.maxResults]    default 200
 */
export async function listCardTransactions(opts = {}) {
  const { reviewStatus, sinceDate, maxResults = 200 } = opts;
  const filters = [];
  if (reviewStatus) filters.push({ field: 'reviewStatus', op: '=', value: reviewStatus });
  if (sinceDate) filters.push({ field: 'transactionDate', op: '>', value: sinceDate });
  const data = await authenticated('/List/ChargeAccountTransaction.json', {
    data: JSON.stringify({
      start: 0,
      max: maxResults,
      filters,
      sort: [{ field: 'transactionDate', asc: '0' }],
    }),
  });
  return data || [];
}
...
/**
 * Mark a charge-card transaction as reviewed with the GL coding we
 * decided on in Breeze. Idempotent — re-running with the same
 * payload is a no-op on Bill.com's side.
 *
 * @param {object} params
 * @param {string} params.billComTransactionId
 * @param {string} params.chartOfAccountsId   Bill.com's GL id
 * @param {string} [params.memo]
 * @param {string} [params.reviewStatus]      defaults 'reviewed'
 */
export async function markCardTransactionReviewed(params) {
  const {
    billComTransactionId,
    chartOfAccountsId,
    memo,
    reviewStatus = 'reviewed',
  } = params;
  if (!billComTransactionId) throw new Error('markCardTransactionReviewed: billComTransactionId required');
  if (!chartOfAccountsId) throw new Error('markCardTransactionReviewed: chartOfAccountsId required');

  const data = await authenticated('/Crud/Update/ChargeAccountTransaction.json', {
    data: JSON.stringify({
      obj: {
        entity: 'ChargeAccountTransaction',
        id: billComTransactionId,
        chartOfAccountsId,
        reviewStatus,
