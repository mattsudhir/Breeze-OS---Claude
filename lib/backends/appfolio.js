...
// Database API v0. The Reports API is gated by a Rails-style
// `authenticate_or_request_with_http_basic` block using a username/
// password pair configured per-customer in the AppFolio web UI
// (Tools → Database / API), NOT in the Developer Space.
//
// When APPFOLIO_REPORTS_USERNAME and APPFOLIO_REPORTS_PASSWORD are
// set, we use them for Reports API requests. If not set, we fall
// back to the Database API credentials so calls fail with a clear
// 401 (rather than throwing) and the introspect endpoint can surface
// the misconfiguration in its response.
function getReportsAuthHeaders() {
  const reportsUser = process.env.APPFOLIO_REPORTS_USERNAME;
  const reportsPass = process.env.APPFOLIO_REPORTS_PASSWORD;

  if (reportsUser && reportsPass) {
    const credentials = Buffer.from(`${reportsUser}:${reportsPass}`).toString('base64');
    const headers = {
      'Authorization': `Basic ${credentials}`,
      'Accept': 'application/json',
    };
...
export async function probeReportsEndpoints() {
  ...
  const usingReportsCreds = Boolean(
    process.env.APPFOLIO_REPORTS_USERNAME && process.env.APPFOLIO_REPORTS_PASSWORD,
  );
  ...
  return {
    ...
    auth_hint: usingReportsCreds
      ? 'Reports API basic auth = APPFOLIO_REPORTS_USERNAME / APPFOLIO_REPORTS_PASSWORD'
      : 'Reports API basic auth falling back to APPFOLIO_CLIENT_ID / APPFOLIO_CLIENT_SECRET — these are likely WRONG for the Reports API. Set APPFOLIO_REPORTS_USERNAME and APPFOLIO_REPORTS_PASSWORD env vars from AppFolio UI.',
    variants: results,
  };
}
