...
const TOOLS = [
  ...
  {
    // Renamed from `list_gl_accounts` to avoid the collision with the
    // Database API tool of the same name above (line ~697). Anthropic
    // rejects the whole request if any two tool names collide; this
    // one had been silently shadowed at execution time by the JS
    // switch-case (top-to-bottom) but its mere presence in the tools
    // array was the cause of the 400 the chat surfaced.
    name: 'list_chart_of_accounts',
    description:
      'List the full chart of accounts from AppFolio Reports API. ' +
      'Returns account numbers, names, types, and any account hierarchy ' +
      'AppFolio exposes. Prefer the simpler `list_gl_accounts` (Database ' +
      'API) for ad-hoc lookups; use this one when you need the structured ' +
      'COA shape for a report-style response.',
    input_schema: { type: 'object', properties: {}, required: [] },
  },
  {
