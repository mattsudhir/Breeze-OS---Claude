...
// Database API base URL. Defaults to the shared host AppFolio
// documents; can be overridden via APPFOLIO_DATABASE_API_URL when a
// customer's Database API runs on their own subdomain
// (e.g. https://acme.appfolio.com/api/v0).
const BASE_URL =
  (process.env.APPFOLIO_DATABASE_API_URL || '').trim() ||
  'https://api.appfolio.com/api/v0';
// AppFolio Reports API v1/v2 lives under a per-tenant subdomain
// (https://<subdomain>.appfolio.com/api/v2/reports/<name>.json).
// Defaults to Breeze's; overridable via APPFOLIO_DATABASE_SUBDOMAIN
// when migrating to a different account.
