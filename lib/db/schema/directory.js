...
export const properties = pgTable(
  'properties',
  {
    ...
    // Stable external property ID from the source PMS. RentManager
    // used integers; AppFolio uses UUIDs — so this is text, not
    // integer (widened in migration 0031). Renamed from rm_property_id
    // in migration 0003.
...
export const units = pgTable(
  'units',
  {
    ...
    notes: text('notes'),
    // Breeze-native flag: exclude this unit from occupancy
    // calculations (still counted for accounting). Backfilled in
    // migration 0038 from a maintained property-exclusion list +
    // "common" name pattern + AppFolio NonRevenue flag.
    nonRevenue: boolean('non_revenue').notNull().default(false),
    createdAt: timestamp('created_at', { withTimezone: true }).defaultNow().notNull(),
    updatedAt: timestamp('updated_at', { withTimezone: true }).defaultNow().notNull(),
  },
  (t) => ({
