...
export const organizations = pgTable('organizations', {
  ...
  // Setup-wizard state. Null for orgs created before the wizard
  // landed — UI treats null as "already onboarded". Shape:
  //   { current_step, completed_steps[], started_at, completed_at,
  //     skipped_steps[] }
  onboardingState: jsonb('onboarding_state'),
  // AI autonomy default — how much the AI is allowed to do without a
  // human in the loop. Each ai_workflow can override; null on the
  // workflow inherits this value.
  aiDefaultAutonomyLevel: text('ai_default_autonomy_level')
    .notNull()
    .default('approve_before_action'),
  // Where the bookkeeper reviews categorized charges:
  //   'breeze'   — review in Breeze; Bill.com is execution-only
  //   'bill_com' — review in Bill.com; Breeze writes GL coding back
  //   'both'     — review in Breeze AND sync to Bill.com (transition)
  bookkeeperReviewLocation: text('bookkeeper_review_location')
    .notNull()
    .default('breeze'),
