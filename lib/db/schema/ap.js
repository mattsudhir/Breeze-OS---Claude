...
export const vendors = pgTable(
  'vendors',
  {
    ...
    // Bill.com mapping (migration 0026). Set per-vendor after the
    // vendor is created in Bill.com.
    billComVendorId: text('bill_com_vendor_id'),
...
export const billPayments = pgTable(
  'bill_payments',
  {
    ...
    memo: text('memo'),
    // Bill.com tracking (added in migration 0026). Populated when
    // payment_method = 'bill_com'. status here is Bill.com's view
    // ('Scheduled' | 'Sent' | 'Cleared' | 'Voided' | etc.); our
    // bill_payments.status above is the JE-level "we committed" state.
    billComPaymentId: text('bill_com_payment_id'),
    billComStatus: text('bill_com_status'),
    billComSyncedAt: timestamp('bill_com_synced_at', { withTimezone: true }),
    createdAt: timestamp('created_at', { withTimezone: true }).defaultNow().notNull(),
    updatedAt: timestamp('updated_at', { withTimezone: true }).defaultNow().notNull(),
  },
  (t) => ({
    ...
    billComIdIdx: index('bill_payments_bill_com_id_idx').on(t.billComPaymentId),
  }),
);
