...
export const properties = pgTable(
  'properties',
  {
    ...
    rmPropertyId: integer('rm_property_id'), // FK into Rent Manager
  ...
  (t) => ({
    ...
    rmIdx: index('properties_rm_idx').on(t.rmPropertyId),
  }),
);

// Per-property utility configuration — the source of truth for
// "who's supposed to hold this utility" and "which provider services it".
export const propertyUtilities = pgTable(
  'property_utilities',
  {
    ...
    // Even when account_holder is 'owner_llc', the tenant may be charged
    // for usage via billback (typical for SFR water). This flag drives
    // a separate billback workflow — out of scope for the first version
    // but captured here so it's ready when we build it.
...
export const moveEvents = pgTable(
  'move_events',
  {
    ...
    rmTenantId: integer('rm_tenant_id'), // FK into Rent Manager
    tenantDisplayName: text('tenant_display_name'), // snapshot, in case RM record changes
  ...
  (t) => ({
    ...
    rmTenantIdx: index('move_events_rm_tenant_idx').on(t.rmTenantId),
  }),
);
