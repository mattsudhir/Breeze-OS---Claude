...
import { formatChatMetricReply } from './chatMetrics/replyTemplates.js';
...
export async function runAgent(
  ...
  while (iterations < MAX_ITERATIONS) {
    ...
    if (response.stop_reason === 'tool_use') {
      ...
      // Fast-path: if the only tool the agent called was
      // get_chat_metric and it returned cleanly, skip the second
      // LLM round-trip and template the answer locally. Cuts simple
      // "how many X" questions from ~6-10s to ~3-5s.
      if (toolUses.length === 1 && toolUses[0].name === 'get_chat_metric') {
        const onlyCall = toolUses[0];
        const onlyResult = JSON.parse(toolResults[0].content);
        if (onlyResult && onlyResult.ok && onlyResult.value) {
          const templated = formatChatMetricReply(
