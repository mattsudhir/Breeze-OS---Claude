  ...
  try {
    switch (name) {
      ...
      case 'make_call': {
        ...
        // is calling and what it needs to accomplish.
        const systemPrompt = [
          `You are ${callerName}, a representative from ${companyName}. You are making an outbound phone call.`,
          `You are calling ${name}.`,
          `Your goal for this call: ${purpose}`,
          contextInfo ? `Background: ${contextInfo}` : '',
          '',
          'Conversation style:',
          `- Wait for the recipient to answer the phone and greet you (e.g. "Hello?") BEFORE you speak. Do not say anything until they have spoken first.`,
          `- Once they have greeted you, respond naturally: "Hi, this is ${callerName} from ${companyName}. Am I speaking with ${name}?"`,
          '- Speak as a real human representative would. Do NOT mention being an AI, a bot, an assistant, or an automated system. Do not say "I am an AI" or "this is an automated call" or anything similar under any circumstances.',
          ...
          '- If you reach voicemail, leave a brief professional message identifying yourself as ' + callerName + ' from ' + companyName + ' and ask them to call back.',
          '- If the person disputes amounts or dates, acknowledge their concern and let them know you will have someone follow up.',
          '- End the call gracefully once the goal is achieved or the person asks to end it. Thank them for their time.',
        ].filter(Boolean).join('\n');
        ...
        //
        // Also clear tools[] — if the base Vapi assistant has orphaned tool
        // references (deleted tools still listed in its config), Vapi will
        // 400 with "Couldn't get tool for hook" before the call starts.
        // Passing an empty tools array in the override wipes them for this
        // call without touching the base assistant.
        const modelConfig = {
          provider: 'anthropic',
