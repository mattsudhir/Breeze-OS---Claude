    ...
    switch (name) {
      ...
      case 'make_call': {
        ...
        const systemPrompt = [
          ...
          '- If you reach voicemail, leave a brief professional message identifying yourself as ' + callerName + ' from ' + companyName + ' and ask them to call back. Then use the endCall tool to hang up.',
          '- If the person disputes amounts or dates, acknowledge their concern and let them know you will have someone follow up.',
          '- When the goal has been achieved, or the recipient says goodbye, or the conversation has naturally concluded, thank them for their time, say goodbye, and then USE THE endCall TOOL to hang up. Do not keep the line open after saying goodbye.',
          '- Never leave a call open after the conversation is over. Hanging up cleanly is part of a professional call.',
        ].filter(Boolean).join('\n');
