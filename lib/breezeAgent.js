...
//   DATABASE_URL           – used by the 'breeze' (production) backend
...
const COMMON_TOOL_NAMES = new Set(['notify_team', 'make_call']);
...
export async function runAgent(
  inputMessages,
  { clientOverride, systemPrompt, dataSource } = {},
) {
  ...
  while (iterations < MAX_ITERATIONS) {
    ...
    if (response.stop_reason === 'tool_use') {
      ...
      for (const call of toolUses) {
        const result = await executeTool(call.name, call.input || {}, backend);
        toolResults.push({
          type: 'tool_result',
          tool_use_id: call.id,
