...
import { getMetric } from './chatMetrics/read.js';
import { METRIC_KEYS, getMetricDef } from './chatMetrics/registry.js';
import { formatChatMetricReply, formatCountMaintenanceReply } from './chatMetrics/replyTemplates.js';
import { countMaintenance } from './chatMetrics/countMaintenance.js';
import { getDefaultOrgIdForMirror } from './appfolioMirror.js';
...
export const COMMON_TOOLS = [
  ...
  {
    name: 'get_chat_metric',
    description:
      "Read a pre-computed answer from the chat_metrics cache. ALWAYS try this BEFORE the slow count_* / list_* tools when the user asks a 'how many X' / 'what is X' style question — it returns in ~50ms versus 5-30s for a fresh AppFolio scan. " +
      "Available metric_keys (org-scoped unless noted): " +
      "'tenant_count' (total/active/hidden tenant records), " +
      "'property_count', 'unit_count' (with by_status breakdown), " +
      "'occupancy_pct' (returns {pct, active_tenants, rentable_units} — this is the right answer for 'how many tenants' style questions; phrase as 'X tenancies out of Y units'), " +
      "'vacant_unit_count', " +
      "'open_maint_count', 'urgent_maint_count' (Urgent + Emergency ONLY — does NOT include High), 'maint_by_priority' (full priority breakdown), 'stale_maint_count' (open >30 days), 'maint_by_category' (HVAC/plumbing/electrical/…), " +
      "'delinquent_tenant_count', 'total_delinquency_cents', " +
      "'tenant_balance_cents' (per-tenant — pass scope_id = AppFolio tenant id), " +
      "'tenant_lease_summary' (per-tenant — pass scope_id = AppFolio tenant id). " +
      "If the metric you need isn't listed, fall back to the count_* / list_* / search_* tools as normal. " +
      "If the cache returns 'error: Unknown metric_key', that key doesn't exist yet — pick a different tool, do NOT retry.",
    input_schema: {
      type: 'object',
      properties: {
        metric_key: {
          type: 'string',
          description: 'One of the metric_keys listed in the tool description.',
        },
        scope_id: {
          type: 'string',
          description:
            'Required for per-tenant / per-property scoped metrics (the AppFolio resource id). Omit for org-scoped metrics.',
        },
      },
      required: ['metric_key'],
    },
  },
  {
    ...
    description:
      "Count open/completed work orders with combined filters that the chat_metrics keys can't express alone. " +
      "Hits the local AppFolio work_order mirror — ~100ms, no AppFolio round-trip. " +
      "Use this for ANY question that crosses two maintenance dimensions, e.g. " +
      "'how many urgent HVAC tickets', 'stale plumbing work orders', 'open tickets mentioning mold'. " +
      "Do NOT use for plain 'how many open work orders' — that's open_maint_count and is faster. " +
      "If a single chat_metric covers the question (open_maint_count, urgent_maint_count, stale_maint_count, " +
      "maint_by_priority, maint_by_category), prefer get_chat_metric instead.",
    input_schema: {
      type: 'object',
      properties: {
        status: {
          ...
          description: "Default 'open'. 'open' = anything not Completed/Canceled/Closed.",
        },
        min_priority: {
          type: 'string',
          enum: ['urgent', 'high', 'medium', 'low'],
          description:
            "Minimum priority bucket. 'urgent' = Urgent+Emergency only. 'high' = those + High. Etc.",
        },
        category: {
          type: 'string',
          enum: [
          ...
          description:
            "Trade/category bucket. Matches against VendorTrade + summary + description text.",
        },
        search: {
          type: 'string',
          description:
            'Free-text keyword (ILIKE) matched against summary + description.',
        },
        stale_only: {
          type: 'boolean',
          description: 'If true, restrict to open tickets reported >30 days ago.',
        },
      },
      required: [],
    },
...
export async function executeTool(name, input, backend = null) {
  ...
  try {
    switch (name) {
      ...
      case 'count_maintenance': {
        try {
          const orgId = await getDefaultOrgIdForMirror();
          const out = await countMaintenance({
            orgId,
            status: input.status,
            min_priority: input.min_priority,
            category: input.category,
            search: input.search,
            stale_only: input.stale_only,
          });
          return { ok: true, ...out };
        } catch (err) {
          return { error: `count_maintenance failed: ${err.message}` };
        }
      }

      case 'get_chat_metric': {
        // Reads from chat_metrics (pre-computed answers). See ADR 0006.
        // Stays inside the process — no HTTP round-trip — because the
        // reader hits Postgres directly via Drizzle.
        const key = (input.metric_key || '').toString().trim();
        if (!key) return { error: 'metric_key is required' };
        const def = getMetricDef(key);
        if (!def) {
          return {
            error: `Unknown metric_key: ${key}. Valid: ${METRIC_KEYS.join(', ')}`,
          };
        }
        try {
          const orgId = await getDefaultOrgIdForMirror();
          const out = await getMetric(orgId, key, input.scope_id || '');
          return out;
        } catch (err) {
          return { error: `chat_metric read failed: ${err.message}` };
        }
      }
...
Unclear / garbled input (IMPORTANT — voice STT can mishear words):
- If the user's question contains an obviously misheard word that doesn't map to any concept you understand (e.g. "urgent quarters" — quarters isn't a property-management term; the user likely said "work orders" or "tickets"), respond IMMEDIATELY with a one-sentence clarifying question. Do NOT call tools to guess. Example: "Did you mean 'urgent work orders'? I can pull that count if so."
- If a question has multiple plausible interpretations, pick the most common one + state it, don't loop on tool calls. Example: "Pulling urgent work orders — let me know if you meant something else."
- Bail-fast rule: ONE tool call max for ambiguous input. If the first tool returns nothing useful, ASK rather than calling another tool.
- This rule overrides "prefer tools over guessing" for ambiguous input. Silence + multi-tool loops are worse than a 1-second clarification.
...
Fast metrics (chat_metrics cache — PREFER THIS for any "how many X" or "what is X" type question):
- ALWAYS try get_chat_metric BEFORE count_* / list_* tools. The cache returns in ~50ms; AppFolio scans take 5-30s.
- Available metric_keys: tenant_count, property_count, unit_count, occupancy_pct, vacant_unit_count, open_maint_count, urgent_maint_count, stale_maint_count, maint_by_category, delinquent_tenant_count, total_delinquency_cents, tenant_balance_cents (scoped), tenant_lease_summary (scoped).

Combined-dimension maintenance — use count_maintenance whenever the question mentions BOTH a priority AND a category, or BOTH a status AND a keyword, etc. Examples that MUST use count_maintenance:
- "how many urgent HVAC tickets" → count_maintenance(min_priority=urgent, category=hvac)
- "open plumbing work orders" → count_maintenance(status=open, category=plumbing)
- "stale electrical issues" → count_maintenance(stale_only=true, category=electrical)
- "any open tickets mentioning mold" → count_maintenance(search=mold)
- "high-priority leaks" → count_maintenance(min_priority=high, search=leak)

Only use get_chat_metric for single-dimension maintenance questions ("how many open work orders", "how many urgent", "stale tickets"). The moment a second filter appears (a trade, a keyword, a property name), switch to count_maintenance — get_chat_metric CANNOT combine dimensions and will give a misleading total.
- If the metric returns {ok: false, error: "Unknown metric_key"}, that key doesn't exist — fall through to count_* / list_*. Do NOT retry the same key.
- If the metric returns a value with computed_at older than the user clearly cares about (e.g. they ask "right now this second"), explicitly fall through to the live tool and mention the cache age.

Tenancy framing (IMPORTANT — applies to "how many tenants" and "what's our occupancy" questions):
- "How many tenants do we have?" is almost always really "how many active tenancies do we have?". A property manager cares about occupancy, not tenant-record count.
- For these questions: call get_chat_metric with metric_key=occupancy_pct. The result has {pct, active_tenancies, rentable_units}. Phrase the answer EXACTLY as "We have X tenancies out of Y units (Z%)." Use those three numbers verbatim. e.g. "We have 524 tenancies out of 626 units (83.7%)."
- "What is our occupancy?" — same answer, same phrasing.
- TRUST the cached value. Do NOT run a second tool call (count_tenants, count_units, list_*) to cross-check. The cache IS the source of truth for these questions. Cross-checking turns a 50ms answer into a 30s answer and adds nothing.
- ONLY answer with raw tenant_count if the user explicitly asks about tenant RECORDS ("how many tenant records", "how big is our tenant database"). For those, get_chat_metric metric_key=tenant_count returns {total, active, hidden} where "active" means non-hidden RECORDS (includes past tenants).
...
Identifier hygiene (read this — applies to every reply):
- Do NOT include work-order numbers, ticket IDs, lease IDs, AppFolio resource ids, or other internal identifiers in replies UNLESS the user explicitly asked for them. They're noise to a human reading on a phone.
- Describe records by what they ARE — address / unit / tenant name / issue — not by their number. Wrong: "WO-57 is overdue." Right: "The leaky-faucet ticket at 14 Southard Apt 3 is overdue."
- If a list of tickets includes IDs in the tool response, drop them when phrasing the answer. The user can tap [SHOWME] to see the full list with IDs in the maintenance view.
- Exception: when the user says "what's the WO number for X" / "give me ticket IDs" / "show me ID 12345", echo the ids verbatim.
...
export async function runAgent(
  ...
  while (iterations < MAX_ITERATIONS) {
    ...
    if (response.stop_reason === 'tool_use') {
      ...
      // Fast-path: skip the second LLM round-trip when the agent's
      // sole tool call was a cache/mirror read with a clean result.
      // Cuts "how many X" questions from ~6-10s to ~3-5s.
      if (toolUses.length === 1) {
        const onlyCall = toolUses[0];
        const onlyResult = JSON.parse(toolResults[0].content);

        let templated = null;
        if (onlyCall.name === 'get_chat_metric' && onlyResult?.ok && onlyResult?.value) {
          templated = formatChatMetricReply(
            (onlyCall.input || {}).metric_key,
            onlyResult.value,
          );
        } else if (onlyCall.name === 'count_maintenance' && onlyResult?.ok) {
          templated = formatCountMaintenanceReply(onlyCall.input || {}, onlyResult);
        }
