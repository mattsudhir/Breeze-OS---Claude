...
//   ZOHO_MCP_SERVER_URL    – Zoho MCP server endpoint (streamable-HTTP). When set,
...
export async function runAgent(inputMessages, { clientOverride, systemPrompt } = {}) {
  ...
  // ── Zoho MCP connector ──────────────────────────────────────────
  // When ZOHO_MCP_SERVER_URL is configured, we pass it to the Anthropic
  // API via the MCP connector beta. The API connects to the Zoho MCP
