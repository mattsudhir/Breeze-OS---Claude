...
// Count rentable units = Breeze units where non_revenue=false.
// The flag is the Breeze-native source of truth, backfilled in
// migration 0038 from a maintained exclusion list + "common" name
// pattern + AppFolio's NonRevenue. Editable per-unit going forward.
  ...
  const rows = await db
    ...
    .where(
      and(
        ...
        eq(schema.units.nonRevenue, false),
      ),
    );
  return rows[0]?.n || 0;
}
