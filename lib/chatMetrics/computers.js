...
export async function occupancyPct(db, orgId) {
  // Definition: active tenants ÷ rentable units (active, not non-revenue).
  const tenants = await tenantCount(db, orgId);
  const units = await unitCount(db, orgId);
  const rentable = Math.max(0, (units.active || 0) - (units.non_revenue || 0));
  if (rentable === 0) {
    return { pct: 0, active_tenants: tenants.active, rentable_units: 0 };
  }
  const pct = Math.min(100, Math.round((tenants.active / rentable) * 1000) / 10);
  return {
    pct,
...
export async function vacantUnitCount(db, orgId) {
  // A vacant unit = active, rentable, with no current occupancy in the
  // mirror. Easiest signal: unit row's status field, falling back to
  // "no current_occupancy_id".
  const rows = await db
    .select({
      n: sql`COUNT(*)::int`.as('n'),
    })
    ...
    .where(
      and(
        ...
        sql`(
          LOWER(${schema.appfolioCache.status}) = 'vacant'
          OR ${schema.appfolioCache.data}->>'current_occupancy_id' IS NULL
        )`,
      ),
    );
  ...
  const rows = await db
    ...
    .where(
      and(
        eq(schema.appfolioCache.organizationId, orgId),
        eq(schema.appfolioCache.resourceType, 'work_order'),
        sql`LOWER(COALESCE(${schema.appfolioCache.data}->>'priority', '')) IN ('urgent', 'emergency', 'high')`,
        sql`${schema.appfolioCache.status} IS NULL
          OR ${schema.appfolioCache.status} NOT IN (${sql.join(CLOSED_WO_STATUSES.map((s) => sql`${s}`), sql`, `)})`,
      ),
    );
...
export async function maintByCategory(db, orgId) {
  // Pull all open work orders, classify in JS so the bucketing rules
  // stay in one place.
  const rows = await db
    .select({
    ...
    .groupBy(sql`${schema.appfolioCache.data}->>'categoryName'`);

  const buckets = {};
  for (const r of rows) {
    const b = categorise(r.category);
    buckets[b] = (buckets[b] || 0) + (r.n || 0);
  }
