...
// "Open work order" filter as a single condition. Wrapping the
// NULL-or-not-in clause via Drizzle's or() guarantees the OR gets
// parenthesised when composed with and() — otherwise SQL precedence
// (AND binds tighter than OR) silently drops adjacent filters.
function openWorkOrderClause() {
  return or(
    isNull(schema.appfolioCache.status),
    sql`${schema.appfolioCache.status} NOT IN (${sql.join(CLOSED_WO_STATUSES.map((s) => sql`${s}`), sql`, `)})`,
  );
}
...
// Count rentable units = Breeze units MINUS non-rentable rows.
// Two exclusion signals, applied together (belt and suspenders):
//   1. Unit name contains "common" (case-insensitive) — common areas
//      like "Common Laundry" / "Common Area" aren't tenancies.
//      This is the more reliable signal in practice.
//   2. AppFolio mirror flags the unit NonRevenue (joined via
//      source_unit_id). Catches anything name-based filtering misses.
// User's directive: "either a non-revenue unit flag … or any unit
// with the word common in the name should also be excluded — they're
...
async function rentableUnitCount(db, orgId) {
  const result = await db.execute(sql`
    ...
    WHERE u.organization_id = ${orgId}
      AND LOWER(COALESCE(u.source_unit_name, '')) NOT LIKE '%common%'
      AND NOT EXISTS (
        SELECT 1
