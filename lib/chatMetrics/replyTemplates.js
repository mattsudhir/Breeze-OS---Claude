...
// Templates a reply for count_maintenance. Phrasing varies based on
// which filters were active so the answer reads naturally without an
// LLM round-trip.
export function formatCountMaintenanceReply(input, result) {
  if (!result || result.count == null) return null;
  const n = fmtNum(result.count);
  const adjectives = [];
  const noun = result.count === 1 ? 'work order' : 'work orders';

  if (input.min_priority === 'urgent') adjectives.push('urgent/emergency');
  else if (input.min_priority === 'high') adjectives.push('high-priority or higher');
  else if (input.min_priority === 'medium') adjectives.push('medium-priority or higher');

  if (input.category) adjectives.push(categoryLabel(input.category));

  const status =
    input.status === 'completed' ? 'completed'
    : input.status === 'all' ? '' : 'open';

  const stale = input.stale_only ? ' older than 30 days' : '';
  const search = input.search ? ` mentioning "${input.search}"` : '';

  // Compose: "There are <N> [adjectives] [status] [noun][ older than 30 days][ mentioning X]."
  const adj = adjectives.length ? ` ${adjectives.join(' ')}` : '';
  const stat = status ? ` ${status}` : '';

  const filterParts = [];
  if (input.status && input.status !== 'all') filterParts.push(`status=${input.status}`);
  if (input.min_priority) filterParts.push(`min_priority=${input.min_priority}`);
  if (input.category) filterParts.push(`category=${input.category}`);
  if (input.search) filterParts.push(`search=${input.search}`);
  const showmeFilters = filterParts.length ? ' ' + filterParts.join(' ') : '';
  const showme = `[SHOWME view=maintenance${showmeFilters}]`;

  return `There are **${n}**${adj}${stat} ${noun}${stale}${search}.\n\n${showme}`;
}
