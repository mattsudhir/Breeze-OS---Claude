      ...
      >
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div
            ...
            title={ticket.title}
          >
            {ticket.title_source === 'ai_summary' && (
              <span
                title="Title generated by Claude from the full work-order description. Editing reverts to manual."
                style={{
                  display: 'inline-flex', alignItems: 'center', gap: 2,
                  padding: '1px 5px', marginRight: 6, borderRadius: 4,
                  fontSize: 9, fontWeight: 700, letterSpacing: '0.04em',
                  background: '#E3F2FD', color: '#1565C0', verticalAlign: 'middle',
                }}
              >
                <Sparkles size={9} /> AI
              </span>
...
export default function MaintenancePage({ initialFilters } = {}) {
  ...
  // Manual trigger for the opt-in AI ticket-title summarization
  // worker (see ADR 0004). Hits /api/admin/summarize-pending-titles
  // with a small per-click cap so cost stays bounded; rerun to
  // process the next batch. Re-loads tickets on success so the AI
  // badges show up immediately.
  const summarizeTitles = async (limit = 50) => {
    setSummarizing(true);
    setSummarizeResult(null);
    try {
      const j = await fetchJson('/api/admin/summarize-pending-titles', {
        method: 'POST',
      ...
      if (j.updated > 0) await load();
    } catch (e) {
      setSummarizeResult({ ok: false, error: e.message });
    } finally {
    ...
    <div style={{ padding: 24, maxWidth: 1200, margin: '0 auto' }}>
      <div style={{
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          ...
          <button
            onClick={() => summarizeTitles(50)}
            disabled={summarizing}
            title="Pick up to 50 long-description tickets that still have the first-sentence title and ask Claude to rewrite them. Manual edits are never touched. See ADR 0004."
            style={{
              padding: '7px 12px', border: '1px solid #6A1B9A', background: '#fff',
              color: '#6A1B9A', borderRadius: 6,
              cursor: summarizing ? 'wait' : 'pointer', fontSize: 13, fontWeight: 600,
              display: 'inline-flex', alignItems: 'center', gap: 5,
            }}
          >
            {summarizing
              ? <><Loader2 size={13} className="spin" /> Summarizing…</>
              : <><Sparkles size={13} /> Summarize titles</>}
          </button>
      ...
      {summarizeResult && (
        <div style={{
          marginBottom: 12,
          padding: '10px 14px', borderRadius: 8, fontSize: 13,
          background: summarizeResult.ok ? '#F3E5F5' : '#FFEBEE',
          border: `1px solid ${summarizeResult.ok ? '#E1BEE7' : '#FFCDD2'}`,
          color: summarizeResult.ok ? '#6A1B9A' : '#C62828',
          display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10,
        }}>
          <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}>
            {summarizeResult.ok
              ? <><Sparkles size={14} /> Considered {summarizeResult.candidates} ticket{summarizeResult.candidates === 1 ? '' : 's'} — {summarizeResult.updated} re-titled by AI, {summarizeResult.skipped} skipped, {summarizeResult.failed} failed. Run again to process the next batch.</>
              : <><AlertCircle size={14} /> Summarize failed: {summarizeResult.error}</>}
          </span>
          <button onClick={() => setSummarizeResult(null)} style={{
            background: 'transparent', border: 'none', cursor: 'pointer',
            color: 'inherit', padding: 2,
          }}><X size={14} /></button>
        </div>
